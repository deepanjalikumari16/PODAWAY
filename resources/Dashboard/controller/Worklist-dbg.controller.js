	sap.ui.define([
		...
		return BaseController.extend("com.coil.podway.Dashboard.controller.Worklist", {
			...
			onInit: function () {
				try {
					jQuery.sap.require("com.coil.podway.Dashboard.libs.mapsjs-core");
					jQuery.sap.require("com.coil.podway.Dashboard.libs.mapsjs-service");
					jQuery.sap.require("com.coil.podway.Dashboard.libs.mapsjs-ui");
					jQuery.sap.require("com.coil.podway.Dashboard.libs.mapsjs-mapevents");
					jQuery.sap.require("com.coil.podway.Dashboard.libs.mapsjs-data");
					jQuery.sap.require("com.coil.podway.Dashboard.libs.mapsjs-clustering");
				} catch (err) {
					console.log(err);
				}

				//set icon of first card
				//	this.getView().byId("card1Image").setSrc($.sap.getModulePath("com/coil/podway/Dashboard", "/css/wheelchair@3x.png"));
			...
			handleNotificationPopoverPress: function (oEvent) {
				...
				if (!this._oPopover) {
					Fragment.load({
						name: "com.coil.podway.Dashboard.dialog.Notifications",
						controller: this
			...
			setLocalModel: function () {
				...
				var oDashboardModel = new JSONModel({
					...
					aNotifications: [],
					bViewBusy: false,
					logo: sap.ui.require.toUrl("com/coil/podway/Dashboard/css/wheelchair.svg"),
					EmergencyLogo: sap.ui.require.toUrl("com/coil/podway/Dashboard/css/wheelchair_1.svg"),
					AlertLogo: sap.ui.require.toUrl("com/coil/podway/Dashboard/css/triangle.svg"),
					bHeapMap: false
				});
			...
			onOpenPODCategoriesDialog: function () {
				...
				if (!that._oDlgChartOption) {
					...
					Fragment.load({
						id: oView.getId(),
						name: "com.coil.podway.Dashboard.dialog.CategoryChart",
						controller: that
